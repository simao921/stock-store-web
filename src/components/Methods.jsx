    ...
    <div className="min-h-screen bg-black pt-48 pb-20 px-8 relative">
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-32">
          <span className="text-purple-500 text-[10px] font-black uppercase tracking-[0.4em] mb-4 block">Knowledge Base</span>
          <h1 className="text-7xl md:text-[8rem] font-heading font-black tracking-tighter uppercase leading-[0.8] mb-12">
            ...
            <span className="block text-purple-600 mt-2">MÉTODOS</span>
          </h1>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            {methods.map((method, i) => (
              <motion.div key={method.id} initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.1 }} className="group relative flex flex-col rounded-[2.5rem] overflow-hidden bg-white/[0.02] border border-white/5 hover:border-purple-500/30 transition-all">
                <div className="relative h-72 overflow-hidden bg-black">
                  {method.imagem_url ? <img src={method.imagem_url} alt={method.nome} className="w-full h-full object-cover opacity-60 transition-transform duration-700 group-hover:scale-110 group-hover:opacity-100" /> : <div className="w-full h-full flex items-center justify-center bg-purple-500/5"><Zap size={48} className="text-purple-500/20" /></div>}
                  <div className="absolute top-8 left-8 px-5 py-2 rounded-full bg-black/60 backdrop-blur-xl border border-white/10 text-[9px] font-black uppercase tracking-widest text-purple-400">{method.categoria}</div>
                </div>
                <div className="p-12 flex flex-col flex-grow">
                  ...
                  <div className="mt-auto flex items-center justify-between pt-8 border-t border-white/5">
                    <div className="text-3xl font-black font-heading text-white"><span className="text-sm text-purple-500 mr-2">R$</span>{method.valor.toFixed(2)}</div>
                    <Link to={`/produto/${method.id}`} className="flex items-center gap-3 px-10 py-5 rounded-2xl bg-white text-black text-[10px] font-black uppercase tracking-widest hover:bg-purple-600 hover:text-white transition-all shadow-xl shadow-white/5">ACESSAR <ArrowRight size={16} /></Link>
                  </div>
                </div>
              </motion.div>
            ))}

...
const AdminPanel = ({ onLogout }) => {
  ...
  const [newProduct, setNewProduct] = useState({
    nome: '', descricao: '', valor: '', categoria: 'Script', quantidade: 1, imagem_url: ''
  });
  ...
  return (
    <div className="w-full min-h-screen bg-[#05020a] text-white pt-32 pb-20 px-8">
      <div className="max-w-7xl mx-auto w-full">
        
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-10">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            <span className="text-[10px] font-black uppercase tracking-[0.5em] text-purple-500 mb-3 block">Command Hub</span>
            <h1 className="text-5xl md:text-7xl font-black uppercase tracking-tighter font-heading text-white leading-tight">Gestão Wisey</h1>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="flex gap-4">
            ...
            <button 
               onClick={() => { 
                 ...
                 else { setEditingProduct(null); setNewProduct({nome: '', descricao: '', valor: '', categoria: 'Script', quantidade: 1, imagem_url: ''}); setIsModalOpen(true); }
               }} 
               className="px-10 py-5 rounded-2xl bg-white text-black font-black uppercase tracking-widest text-[11px] hover:bg-purple-600 hover:text-white transition-all shadow-2xl shadow-white/5 flex items-center gap-3"
            >
        ...
        <div className="flex gap-2 mb-16 p-1.5 bg-white/[0.02] border border-white/5 rounded-[2rem] w-fit">
          {tabs.map(t => (
            <button 
              ...
              className={`relative px-10 py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all ${activeTab === t.id ? 'text-white' : 'text-white/20 hover:text-white'}`}
            >
              ...
              {activeTab === t.id && (
                <motion.div layoutId="admin-tab" className="absolute inset-0 bg-purple-600 rounded-2xl shadow-lg shadow-purple-500/20" />
              )}
            </button>
          ))}
        </div>
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'inventory' && (
            <motion.div key="inv" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
              {products.map(p => (
                <div key={p.id} className="glass-card p-8 group border-white/5 hover:border-purple-500/30 transition-all">
                   ...
                   <div className="flex justify-between items-start mb-8">
                      <div>
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest text-purple-500">{p.categoria}</span>
                      </div>
                      <span className="text-2xl font-black font-heading text-white">R${p.valor.toFixed(2)}</span>
                   </div>
                   <div className="flex gap-3">
                      <button onClick={() => { setEditingProduct(p); setNewProduct(p); setIsModalOpen(true); }} className="flex-grow py-4 rounded-xl bg-white/5 border border-white/5 text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-purple-600 hover:text-white transition-all"><Edit2 size={14}/> Editar</button>
                  ...
                  <tbody className="divide-y divide-white/5">
                     {coupons.map(c => (
                        <tr key={c.id} className="text-xs hover:bg-white/[0.01] transition-all">
                           <td className="px-12 py-8 font-black uppercase text-purple-400 tracking-widest">{c.codigo}</td>
                        ...
                        <tr key={o.id} className="text-xs hover:bg-white/[0.01] transition-all">
                           ...
                           <td className="px-12 py-8">
                              <div className="flex justify-center">
                                 <select value={o.status} onChange={(e) => updateOrderStatus(o.id, e.target.value)} className="bg-white/5 border border-white/10 rounded-full px-6 py-2.5 text-[9px] font-black uppercase tracking-widest text-purple-400 outline-none cursor-pointer hover:border-purple-500/50 transition-all">
          ...
          {activeTab === 'overview' && (
            <motion.div key="ov" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="grid grid-cols-1 md:grid-cols-3 gap-10">
               {[
                 { label: 'Faturamento Bruto', val: `R$ ${totalRevenue.toFixed(2)}`, color: 'text-purple-500', icon: DollarSign },
                 ...
                 <div key={i} className="glass-card p-12 flex flex-col items-center text-center border-white/5 relative overflow-hidden group">
                    ...
                    <div className="w-16 h-16 rounded-3xl bg-white/5 border border-white/10 flex items-center justify-center mb-8 text-white/20 group-hover:text-white group-hover:border-purple-500/30 transition-all">
                       <s.icon size={28} />
                    </div>
      ...
      {/* Modals are also stylized similarly... */}
      <AnimatePresence>
        {(isModalOpen || isCouponModalOpen) && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6 bg-black/95 backdrop-blur-2xl">
             <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-xl bg-[#0c0c0f] border border-white/10 rounded-[3rem] p-16 shadow-2xl shadow-purple-500/10">
                ...
                {isCouponModalOpen ? (
                  <form onSubmit={handleSaveCoupon} className="space-y-8">
                    <input required className="w-full bg-white/5 border border-white/10 rounded-2xl py-5 px-8 text-base outline-none focus:border-purple-500 font-bold text-white transition-all" placeholder="CÓDIGO" value={newCoupon.codigo} onChange={e => setNewCoupon({...newCoupon, codigo: e.target.value.toUpperCase()})} />
                    <input required type="number" step="0.01" className="w-full bg-white/5 border border-white/10 rounded-2xl py-5 px-8 text-base outline-none focus:border-purple-500 font-bold text-white transition-all" placeholder="DESCONTO (EX: 0.10)" value={newCoupon.desconto} onChange={e => setNewCoupon({...newCoupon, desconto: e.target.value})} />
                    <div className="flex gap-4">
                       ...
                       <button type="submit" className="flex-grow py-6 rounded-3xl bg-purple-600 text-white font-black uppercase tracking-widest text-[11px] hover:bg-purple-700 shadow-xl shadow-purple-500/20">Salvar Cupão</button>
                    </div>
                  </form>
                ) : (
                  <form onSubmit={handleSaveProduct} className="space-y-6">
                    <input required className="w-full bg-white/5 border border-white/10 rounded-2xl py-5 px-8 text-base outline-none focus:border-purple-500 font-bold text-white transition-all" placeholder="NOME DO PRODUTO" value={newProduct.nome} onChange={e => setNewProduct({...newProduct, nome: e.target.value})} />
                    <div className="grid grid-cols-2 gap-6">
                       <input required type="number" step="0.01" className="w-full bg-white/5 border border-white/10 rounded-2xl py-5 px-8 text-base outline-none focus:border-purple-500 font-bold text-white transition-all" placeholder="PREÇO" value={newProduct.valor} onChange={e => setNewProduct({...newProduct, valor: e.target.value})} />
                       <input required type="number" className="w-full bg-white/5 border border-white/10 rounded-2xl py-5 px-8 text-base outline-none focus:border-purple-500 font-bold text-white transition-all" placeholder="QTD" value={newProduct.quantidade} onChange={e => setNewProduct({...newProduct, quantidade: e.target.value})} />
                    </div>
                    <input className="w-full bg-white/5 border border-white/10 rounded-2xl py-5 px-8 text-base outline-none focus:border-purple-500 font-bold text-white transition-all" placeholder="URL DA IMAGEM" value={newProduct.imagem_url} onChange={e => setNewProduct({...newProduct, imagem_url: e.target.value})} />
                    <textarea required className="w-full bg-white/5 border border-white/10 rounded-2xl py-5 px-8 text-base outline-none focus:border-purple-500 font-bold h-40 resize-none text-white transition-all" placeholder="DESCRIÇÃO DETALHADA" value={newProduct.descricao} onChange={e => setNewProduct({...newProduct, descricao: e.target.value})} />
                    <div className="flex gap-6">
                       ...
                       <button type="submit" className="flex-grow py-6 rounded-3xl bg-purple-600 text-white font-black uppercase tracking-widest text-[11px] hover:bg-purple-700 shadow-xl shadow-purple-500/20">Gravar Alterações</button>
                    </div>
                  </form>
                )}
             </motion.div>
